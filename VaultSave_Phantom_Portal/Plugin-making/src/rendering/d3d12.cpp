...
bool D3D12::initialize()
{
    ...
    if (FAILED(device->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&this->fence))))
    {
        return false;
    }
    this->fence_value = 1;
...
void D3D12::render_imgui()
{
    ...
    // Wait for the previous frame to finish executing commands using this allocator.
    const UINT64 fence_to_wait = this->fence_value - 1;
    if (this->fence->GetCompletedValue() < fence_to_wait)
    {
        if (FAILED(this->fence->SetEventOnCompletion(fence_to_wait, this->fence_event)))
        {
            return;
        }
    ...
    // Signal the fence for the next frame.
